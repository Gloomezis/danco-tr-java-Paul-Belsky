...
public class OrdersDAO implements IDAO<Orders>{
	...
	public int create(IBaseModel baseModel) throws SQLException {
		...
		return exec.execUpdate(con, 
				...
				+ "values ("+gId+","+hrId+",'"+dArrive+"','"+dDeparture+"';)");	
	...
	public Orders read(int id) throws SQLException {
		...
		return exec.execQuery(con, sql + id+ ";",   	
				new TResultHandler<Orders>() {
					...
					@Override
					public Orders handle(ResultSet result)
							throws SQLException {
						result.next();
						Orders orders = new Orders(result
								.getInt("id"), result.getInt("guest_id"),result.getInt("hotel_room_id"),result.getDate("date_arrive"),result.getDate("date_departure"),result.getBoolean("paid"));
						return orders;
					}
				}
		);
	...
	//+
	@Override
	public Orders readByName(String name) throws SQLException {
	...
	TExecutor exec = new TExecutor();
	return exec.execUpdate(con, "UPDATE  orders SET paid = true where id="+id+";");	

	}
	

	public int getSummToDeparture(int id) throws SQLException {
		...
		sql.append("WHERE orders.guest_id = "+id+" and service.paid=false )AS guest_summ_paid; ");
		
		
		TExecutor exec = new TExecutor();

		return exec.execQuery(con, sql.toString(), new TResultHandler<Integer>() {
