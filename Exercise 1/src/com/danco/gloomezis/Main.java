...
//PrintStream ps = new PrintStream(FILE_PATH_LOG);
//ExceptionLogger exl= new ExceptionLogger();
	...
	//	TxtWorker guestTxtWorker=new TxtWorker(fwGuest);
		...
		serviceService.showPriceServiceAndHotelRoom();
	...
	System.out.println("printing  getted guest"+MainStorage.getInstance().getGuestByName("Paul").toString());
	System.out.println("printing  getted hotel room "+MainStorage.getInstance().getHotelRoomByNumber("101").toString());
	System.out.println("printing  getted service "+MainStorage.getInstance().getServiceByName("Diner").toString());
	
	//guestTxtWorker.writeGuests();
    // guestTxtWorker.readGuests();
	//	e.printStackTrace(ps);
	guestService.writeGuestToFile();
